...
private fun shortestCompletingWord(licensePlate: String, words: Array<String>): String {
    ...
    for (c in licensePlate) {
        if (c.isLetter()) {
            map[c.toLowerCase().toInt() - 'a'.toInt()]++
        }
    }
    var res: String? = null
    for (word in words) {
        ...
        for (c in word) {
            temp[c.toInt() - 'a'.toInt()]++
        }
        if (judge(temp, map)) {
            res = if (res == null) word else if (res.length > word.length) word else res
        }
    }
    return res!!
}

private fun judge(temp: IntArray, map: IntArray): Boolean {
    for (i in 0..25) {
        if (temp[i] < map[i]) {
            return false
        }
