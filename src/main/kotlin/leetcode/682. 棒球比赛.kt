...
private fun calPoints(ops: Array<String>): Int {
    ...
    for (op in ops) {
        when (op) {
            "+" -> scores.addLast(scores[scores.lastIndex] + scores[scores.lastIndex - 1])
            "D" -> scores.addLast(scores[scores.lastIndex] * 2)
            "C" -> scores.removeLast()
            else -> scores.addLast(op.toInt())
        }
    }
    return scores.reduce(Int::plus)
}

fun main() {
    println(calPoints(arrayOf("5", "2", "C", "D", "+")))
    println(calPoints(arrayOf("5", "-2", "4", "C", "D", "9", "+", "+")))
    println(calPoints(arrayOf("1")))
}