...
private fun subdomainVisits(cpdomains: Array<String>): List<String> {
    ...
    for (cpdomain in cpdomains) {
        val sepIndex = cpdomain.indexOf(" ")
        val count = cpdomain.substring(0, sepIndex).toInt()
        var domain = cpdomain.substring(sepIndex + 1)
        map[domain] = count + (map[domain] ?: 0)
