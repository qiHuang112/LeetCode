...
private fun mostCommonWord(paragraph: String, banned: Array<String>): String {
    ...
    while (index < paragraph.length) {
        var start = index
        while (start < paragraph.length && !paragraph[start].isLetter()) {
            start++
        }
        if (start == paragraph.length) break
        var end = start + 1
        while (end < paragraph.length && paragraph[end].isLetter()) {
            end++
        }
        index = end
        val word = paragraph.substring(start, end).toLowerCase()
        val curCount = (map[word] ?: 0) + 1
        map[word] = curCount
        if (curCount > count) {
            res = word
            count = curCount
